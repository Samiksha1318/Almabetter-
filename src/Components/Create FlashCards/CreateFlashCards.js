...
const CreateCards  = () => {
  ...
  const IMAGE_FORMATS = ["image/jpeg", "image/jpg", "image/png"];
  ...
  const address_ref = (el) => {
    if (el && !element_ref.current.includes(el)) {
      element_ref.current.push(el);
    }
  }
  // created a notification to show image validation
  const image_error = (val) => {
    toast.warn(val, {
      position: "top-center",
      autoClose: 1000,
                  ...
                  <input
                    onChange={(event) => {
                      ...
                      if (event.target.files[0]
                        && !IMAGE_FORMATS.includes(event.target.files[0].type)) {
                          image_error('unsupported file format')
                      }
                      else if (event.target.files[0].size > 1024 * 1024 * 10) {
                        image_error('image size is very large')
                      } else if (event.target.files[0].size <= 1024 * 1024 * 10) {
                        ...
                        <div className='flex'>
                          ...
                          <div>
                            ...
                            <TbEdit className='text-[1.8em] text-blue-700 m-2 cursor-pointer hover:text-blue-900'
                              onClick={() => { element_ref.current[index].focus() }} />
                          </div>
                        </div>
                      </div>
                    ))
